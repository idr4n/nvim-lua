...
function M.workdirs(args)
  ...
  opts.actions = {
    ["default"] = function(selected)
      ...
      newcwd = fzf_lua.path.starts_with_separator(newcwd) and newcwd or fzf_lua.path.join({ vim.env.HOME, newcwd })
      ...
      elseif args.nvim_alacritty then
        vim.cmd(
          string.format("execute 'silent !open -na alacritty --args --working-directory %s -e fish -ic nvim'", newcwd)
        )
        return
      end
